...
} = require("@hubspot/api-client/lib/codegen/files");
...
const processCompanies = async (domain, hubId, q) => {
    ...
    while (hasMore) {
        const lastModifiedDate =
            offsetObject.lastModifiedDate || lastPulledDate;
        ...
        data.forEach((company) => {
            ...
            q.push({
                ...
                actionDate:
                    new Date(
                        isCreated ? company.createdAt : company.updatedAt
                    ) - 2000,
                ...actionTemplate,
            });
        });
...
const processContacts = async (domain, hubId, q) => {
    ...
    while (hasMore) {
        ...
        if (!searchResult)
            throw new Error(
                "Failed to fetch contacts for the 4th time. Aborting."
            );
...
/**
 * Get recently created or modified meetings
 */
const processMeetings = async (domain, hubId, q) => {
    ...
    while (hasMore) {
        ...
        const searchObject = {
            ...
            properties: ["hs_meeting_title", "hs_timestamp", "hs_createdate"],
            limit,
            after: offsetObject.after,
        };
        let searchResult = {};
        ...
        const contactAssociations = await fetchMeetingContacts(meetingIds);

        meetings.forEach((meeting) => {
            const contactEmail = contactAssociations[meeting.id];
...
/**
 * Fetch contacts associated with meetings
 */
const fetchMeetingContacts = async (meetingIds) => {
    const contactAssociations = {};
    const batchInputs = meetingIds.map((id) => ({ id }));

    const associationResults = await (
        await hubspotClient.apiRequest({
            method: "post",
            path: "/crm/v3/associations/ENGAGEMENTS/CONTACTS/batch/read",
            body: { inputs: batchInputs },
        })
    ).json();

    const contactIds = [];

    associationResults?.results?.forEach((assoc) => {
        if (assoc.from && assoc.to[0]) {
            const contactId = assoc.to[0].id;
            contactAssociations[assoc.from.id] = contactId;
            contactIds.push(contactId);
        }
    });

    if (contactIds.length > 0) {
        const contactPropertiesResponse = await (
            await hubspotClient.apiRequest({
                method: "post",
        ...
        const contactIdToEmailMap = {};
        contactPropertiesResponse?.results?.forEach((contact) => {
            contactIdToEmailMap[contact.id] = contact.properties.email;
        });

        Object.keys(contactAssociations).forEach((meetingId) => {
            const contactId = contactAssociations[meetingId];
            contactAssociations[meetingId] =
                contactIdToEmailMap[contactId] || null;
        });
    }
